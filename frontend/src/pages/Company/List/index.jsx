...
const CompanyList = () => {
	...
	async function handleDelete(item) {
		ShowConfirm('Atenção', `Confirma a remoção da loja ${item.name}?`, () => handleDeleteConfirm(item.id));
	}
	...
	async function handleDeleteConfirm(id) {
		try {
			...
			setLoading(false);
			showToast.error('Verfique se a loja ainda está vinculada a usuarios, clientes, especialidades e etc... ');
			getValidationErrors(error);
		}
	}
	...
	return (
		<Container title="Lojas" loading={loading ? Boolean(loading) : undefined}>
			...
			<LoadMore onClick={() => setPage(page + 1)} total={total} loadedItens={companies.length} />
		</Container>
	);
};
