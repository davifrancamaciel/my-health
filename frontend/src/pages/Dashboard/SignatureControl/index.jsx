...
import { parseISO, formatDistance, setHours, setMinutes, setSeconds, setMilliseconds, isBefore } from 'date-fns';
...
function SignatureControl({ company }) {
	...
	useEffect(() => {
		...
		const checkDate = setMilliseconds(setSeconds(setMinutes(setHours(parseISO(company.expires_at), 0), 0), 0), 0);
		...
		let companyFormatedObj = {
			...
			expiresFormated: `${formatDistance(parseISO(company.expires_at), new Date(), {
				addSuffix: false,
				locale: pt,
			})}`,
		};
	...
	function verifySatusSignature(companyFormated) {
		if (companyFormated && companyFormated.expired) {
			showToast.error(
				'O tempo de utilização do sistema UPIS Saúde expirou. Entre em contato conosco e renove sua assinatura.'
			);
			dispatch(signOut());
		}
	}
	...
	return (
		<h2>
			...
			<InfoDays expireWarning={companyFormated.expireWarning}>{companyFormated.expiresFormated}</InfoDays>
		</h2>
	);
}
