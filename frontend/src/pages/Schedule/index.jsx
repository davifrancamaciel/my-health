...
function Shedule() {
  ...
  useEffect(() => {
    ...
    if (
      notification &&
      isEqual(startOfDay(date), startOfDay(parseISO(notification.date))) &&
      !notification.read
    ) {
      loadSchedule(date);
    }
  ...
  async function handleCancelAppointmentConfirmed(schedule) {
    try {
      ...
      const date = utcToZonedTime(
        parseISO(schedule.appointment.date),
        timezone
      );

      const dateFormatedComplete = format(
        date,
      ...
      const response = await api.delete(
        `appointments/${schedule.appointment.id}`,
        {
      ...
      loadSchedule(date);
      showToast.success(
        `Agendamento de ${schedule.time} CANCELADO com sucesso`
      );
    } catch (error) {
      setLoading(false);
  ...
  function addNotification(appointment) {
    ...
    const userId =
      profile.id === appointment.provider_id
        ? appointment.user_id
          ...
          {schedules.map(schedule => (
            ...
            >
              <div onClick={() => onClickDetails(schedule)}>
                ...
                {schedule.scheduledWithUser && (
                  <span>{schedule.appointment.speciality.type.name}</span>
                )}
              </div>
              {schedule.scheduledWithUser && (
                <img
